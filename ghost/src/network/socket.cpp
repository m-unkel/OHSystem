...
void CSocket :: Allocate( int type )
{
    ...
    if( m_Socket == INVALID_SOCKET )
    {
        ...
        //Log->Write( "[SOCKET] error (socket) - " + GetErrorString( ) );
        return;
    }
}
    ...
    if( FD_ISSET( m_Socket, fd ) )
    {
        ...
        if(c!=0) {
          ...
          m_LastRecv = GetTime( );
            ...
            Log->Write( "[TCPSOCKET] closed by remote host" );
            m_Connected = false;
        }
    }
}
...
void CTCPSocket :: DoRecv( fd_set *fd )
{
    ...
    if( FD_ISSET( m_Socket, fd ) )
    {
        ...
        else if( c == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
        {
            ...
            m_HasError = true;
            m_Error = GetLastError( );
            //Log->Write( "[TCPSOCKET] error (recv) - " + GetErrorString( ) );
            return;
        }
        else if( c == 0 )
        {
...
void CTCPSocket :: DoSend( fd_set *send_fd )
{
    ...
    if( FD_ISSET( m_Socket, send_fd ) )
    {
        ...
        else if( s == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
        {
            ...
            m_HasError = true;
            m_Error = GetLastError( );
            Log->Write( "[TCPSOCKET] error (send) - " + GetErrorString( ) );
            return;
        }
    }
}
...
void CTCPClient :: Connect( string localaddress, string address, uint16_t port )
{
    ...
    if( !localaddress.empty( ) )
    {
        ...
        if( ::bind( m_Socket, (struct sockaddr *)&LocalSIN, sizeof( LocalSIN ) ) == SOCKET_ERROR )
        {
            m_HasError = true;
            m_Error = GetLastError( );
            //Log->Write( "[TCPCLIENT] error (bind) - " + GetErrorString( ) );
            return;
        }
    }
    ...
    if( !HostInfo )
    {
        m_HasError = true;
        // m_Error = h_error;
        //Log->Write( "[TCPCLIENT] error (gethostbyname)" );
        return;
    }
    ...
    if( connect( m_Socket, (struct sockaddr *)&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
    {
        if( GetLastError( ) != EINPROGRESS && GetLastError( ) != EWOULDBLOCK )
        {
            ...
            m_HasError = true;
            m_Error = GetLastError( );
            //Log->Write( "[TCPCLIENT] error (connect) - " + GetErrorString( ) );
            return;
        }
    }
...
bool CTCPServer :: Listen( string address, uint16_t port )
{
    ...
    if( ::bind( m_Socket, (struct sockaddr *)&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
    {
        m_HasError = true;
        m_Error = GetLastError( );
        //Log->Write( "[TCPSERVER] error (bind) - " + GetErrorString( ) );
        return false;
    }
    ...
    if( listen( m_Socket, 8 ) == SOCKET_ERROR )
    {
        m_HasError = true;
        m_Error = GetLastError( );
        //Log->Write( "[TCPSERVER] error (listen) - " + GetErrorString( ) );
        return false;
    }
...
bool CUDPSocket :: SendTo( string address, uint16_t port, BYTEARRAY message )
{
    ...
    if( !HostInfo )
    {
        m_HasError = true;
        // m_Error = h_error;
        //Log->Write( "[UDPSOCKET] error (gethostbyname)" );
        return false;
    }
...
bool CUDPSocket :: Broadcast( uint16_t port, BYTEARRAY message )
{
    ...
    if( sendto( m_Socket, MessageString.c_str( ), MessageString.size( ), 0, (struct sockaddr *)&sin, sizeof( sin ) ) == -1 )
    {
        //Log->Write( "[UDPSOCKET] failed to broadcast packet (port " + UTIL_ToString( port ) + ", size " + UTIL_ToString( MessageString.size( ) ) + " bytes)" );
        return false;
    }
...
void CUDPSocket :: SetBroadcastTarget( string subnet )
{
    if( subnet.empty( ) )
    {
        //Log->Info( "[UDPSOCKET] using default broadcast target" );
        m_BroadcastTarget.s_addr = INADDR_BROADCAST;
    }
    else
    {
        ...
        //Log->Info( "[UDPSOCKET] using broadcast target [" + subnet + "]" );
        ...
        {
            //Log->Warning( "[UDPSOCKET] invalid broadcast target, using default broadcast target" );
            m_BroadcastTarget.s_addr = INADDR_BROADCAST;
        }
    }
}
...
bool CUDPServer :: Bind( struct sockaddr_in sin )
{
    ...
    if( ::bind( m_Socket, (struct sockaddr *)&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
    {
        m_HasError = true;
        m_Error = GetLastError( );
        //Log->Write( "[UDPSERVER] error (bind) - " + GetErrorString( ) );
        return false;
    }
...
void CUDPServer :: RecvFrom( fd_set *fd, struct sockaddr_in *sin, string *message )
{
    ...
    if( FD_ISSET( m_Socket, fd ) )
    {
        ...
        else if( c == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
        {
            ...
            m_HasError = true;
            m_Error = GetLastError( );
            //Log->Write( "[UDPSERVER] error (recvfrom) - " + GetErrorString( ) );
        }
    }
}
