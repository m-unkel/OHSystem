...
void CReplay :: BuildReplay( string gameName, string statString, uint32_t war3Version, uint16_t buildNumber )
{
    ...
    Log->Info( "[REPLAY] building replay" );
...
void CReplay :: ParseReplay( bool parseBlocks )
{
    ...
    if( m_Flags != 32768 )
    {
        Log->Write( "[REPLAY] invalid replay (flags mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage4 != 272 )
    {
        Log->Write( "[REPLAY] invalid replay (4.0 Unknown mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage1 != 0 )
    {
        Log->Write( "[REPLAY] invalid replay (4.1 Host RecordID mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( m_HostPID > 15 )
    {
        Log->Write( "[REPLAY] invalid replay (4.1 Host PlayerID is invalid)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage1 != 1 )
    {
        Log->Write( "[REPLAY] invalid replay (4.1 Host AdditionalSize mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage1 != 0 )
    {
        Log->Write( "[REPLAY] invalid replay (4.1 Host AdditionalData mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( m_PlayerCount > 12 )
    {
        Log->Write( "[REPLAY] invalid replay (4.6 PlayerCount is invalid)" );
        m_Valid = false;
        return;
    }
    ...
    while( 1 )
    {
        ...
        if( Garbage1 == 22 )
        {
            ...
            if( PlayerID > 15 )
            {
                Log->Write( "[REPLAY] invalid replay (4.9 Player PlayerID is invalid)" );
                m_Valid = false;
                return;
            }
            ...
            if( Garbage1 != 1 )
            {
                Log->Write( "[REPLAY] invalid replay (4.9 Player AdditionalSize mismatch)" );
                m_Valid = false;
                return;
            }
            ...
            if( Garbage1 != 0 )
            {
                Log->Write( "[REPLAY] invalid replay (4.9 Player AdditionalData mismatch)" );
                m_Valid = false;
                return;
            }
            ...
            if( Garbage4 != 0 )
            {
                Log->Write( "[REPLAY] invalid replay (4.9 Unknown mismatch)" );
                m_Valid = false;
                return;
            }
        ...
        {
            Log->Write( "[REPLAY] invalid replay (4.9 Player RecordID mismatch)" );
            m_Valid = false;
            return;
        }
    }
    ...
    {
        Log->Write( "[REPLAY] invalid replay (4.10 Size is invalid)" );
        m_Valid = false;
        return;
    }
    ...
    {
        Log->Write( "[REPLAY] invalid replay (4.10 NumSlots is invalid)" );
        m_Valid = false;
        return;
    }
    ...
    if( ISS.eof( ) || ISS.fail( ) )
    {
        Log->Write( "[SAVEGAME] failed to parse replay header" );
        m_Valid = false;
        return;
    }
    ...
    {
        Log->Write( "[REPLAY] invalid replay (5.0 first start block ID mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage4 != 1 )
    {
        Log->Write( "[REPLAY] invalid replay (5.0 first start block data mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    {
        Log->Write( "[REPLAY] invalid replay (5.0 second start block ID mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( Garbage4 != 1 )
    {
        Log->Write( "[REPLAY] invalid replay (5.0 second start block data mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    while( 1 )
    {
        ...
        if( ISS.eof( ) || ISS.fail( ) )
        {
            Log->Write( "[REPLAY] invalid replay (5.0 third start block unexpected end of file found)" );
            m_Valid = false;
            return;
        }
        ...
        {
            Log->Write( "[REPLAY] invalid replay (5.0 third start block ID mismatch)" );
            m_Valid = false;
            return;
        }
    }
    ...
    if( Garbage4 != 1 )
    {
        Log->Write( "[REPLAY] invalid replay (5.0 third start block data mismatch)" );
        m_Valid = false;
        return;
    }
    ...
    if( ISS.eof( ) || ISS.fail( ) )
    {
        Log->Write( "[SAVEGAME] failed to parse replay start blocks" );
        m_Valid = false;
        return;
    }
    ...
    while( 1 )
    {
        ...
        else if( Garbage1 == CReplay :: REPLAY_CHATMESSAGE )
        {
            ...
            if( PID > 15 )
            {
                Log->Write( "[REPLAY] invalid replay (5.0 chatmessage pid is invalid)" );
                m_Valid = false;
                return;
            }
        ...
        else if( Garbage1 == CReplay :: REPLAY_CHECKSUM )
        {
            ...
            if( Garbage1 != 4 )
            {
                Log->Write( "[REPLAY] invalid replay (5.0 checksum unknown mismatch)" );
                m_Valid = false;
                return;
            }
    ...
    if( m_ReplayLength != ActualReplayLength )
        Log->Warning( "[REPLAY] warning - replay length mismatch (" + UTIL_ToString( m_ReplayLength ) + "ms/" + UTIL_ToString( ActualReplayLength ) + "ms)" );
