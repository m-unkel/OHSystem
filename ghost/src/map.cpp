...
{
    //Log->Debug( "[MAP] using hardcoded Emerald Gardens map data for Warcraft 3 version 1.24 & 1.24b" );
...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
    ...
    {
        //Log->Debug( "[MAP] loading MPQ file [" + MapMPQFileName + "]" );
        MapMPQReady = true;
    }
//	else
//		Log->Debug( "[MAP] warning - unable to load MPQ file [" + MapMPQFileName + "]" );
    ...
    if( !m_MapData.empty( ) )
    {
        ...
        //Log->Debug( "[MAP] calculated map_size = " + UTIL_ByteArrayToDecString( MapSize ) );
        ...
        //Log->Debug( "[MAP] calculated map_info = " + UTIL_ByteArrayToDecString( MapInfo ) );
        ...
        if( CommonJ.empty( ) )
            Log->Write( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_OHBot->m_MapCFGPath + "common.j]" );
        else
        {
            string BlizzardJ = UTIL_FileRead( m_OHBot->m_MapCFGPath + "blizzard.j" );

            if( BlizzardJ.empty( ) )
                Log->Write( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_OHBot->m_MapCFGPath + "blizzard.j]" );
            else
            {
                uint32_t Val = 0;
                ...
                if( MapMPQReady )
                {
                    ...
                    if( SFileOpenFileEx( MapMPQ, "Scripts\\blizzard.j", 0, &SubFile ) )
                    {
                        ...
                        if( FileLength > 0 && FileLength != 0xFFFFFFFF )
                        {
                            ...
                            if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead ) )
                            {
                                //Log->Info( "[MAP] overriding default blizzard.j with map copy while calculating map_crc/sha1" );
                ...
                if( MapMPQReady )
                {
                    ...
                    for( vector<string> :: iterator i = FileList.begin( ); i != FileList.end( ); ++i )
                    {
                        ...
                        {
                            // Log->Debug( "*** not found: " + *i );
                        }
                    }

                    //if( !FoundScript )
                    //	Log->Write( "[MAP] couldn't find war3map.j or scripts\\war3map.j in MPQ file, calculated map_crc/sha1 is probably wrong" );

                    MapCRC = UTIL_CreateByteArray( Val, false );
                    //Log->Write( "[MAP] calculated map_crc = " + UTIL_ByteArrayToDecString( MapCRC ) );
                    ...
                    //Log->Info( "[MAP] calculated map_sha1 = " + UTIL_ByteArrayToDecString( MapSHA1 ) );
                }
//				else
//					Log->Write( "[MAP] unable to calculate map_crc/sha1 - map MPQ file not loaded" );
            }
        }
    }
//	else
//		Log->Warning( "[MAP] no map data available, using config file for map_size, map_info, map_crc, map_sha1" );
                ...
                if( FileLength > 0 && FileLength != 0xFFFFFFFF )
                {
                    ...
                    if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead ) )
                    {
                        ...
                        if( FileFormat == 18 || FileFormat == 25 )
                        {
                            ...
                            //Log->Debug( "[MAP] calculated map_options = " + UTIL_ToString( MapOptions ) );
                            MapWidth = UTIL_CreateByteArray( (uint16_t)RawMapWidth, false );
                            //Log->Debug( "[MAP] calculated map_width = " + UTIL_ByteArrayToDecString( MapWidth ) );
                            MapHeight = UTIL_CreateByteArray( (uint16_t)RawMapHeight, false );
                            //Log->Debug( "[MAP] calculated map_height = " + UTIL_ByteArrayToDecString( MapHeight ) );
                            MapNumPlayers = RawMapNumPlayers - ClosedSlots;
                            //Log->Debug( "[MAP] calculated map_numplayers = " + UTIL_ToString( MapNumPlayers ) );
                            MapNumTeams = RawMapNumTeams;
                            //Log->Debug( "[MAP] calculated map_numteams = " + UTIL_ToString( MapNumTeams ) );

                            uint32_t SlotNum = 1;

                            for( vector<CGameSlot> :: iterator i = Slots.begin( ); i != Slots.end( ); ++i )
                            {
                                //Log->Debug( "[MAP] calculated map_slot" + UTIL_ToString( SlotNum ) + " = " + UTIL_ByteArrayToDecString( (*i).GetByteArray( ) ) );
                                ++SlotNum;
                            }

                            if( MapOptions & MAPOPT_MELEE )
                            {
                                //Log->Debug( "[MAP] found melee map, initializing slots" );
...
//					else
//						Log->Debug( "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - unable to extract war3map.w3i from MPQ file" );
...
//			else
//				Log->Debug( "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - couldn't find war3map.w3i in MPQ file" );
        }
//		else
//			Log->Debug( "[MAP] unable to calculate map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams - map MPQ file not loaded" );
    }
//	else
//		Log->Debug( "[MAP] no map data available, using config file for map_options, map_width, map_height, map_slot<x>, map_numplayers, map_numteams" );
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_size with config value map_size = " + CFG->GetString( "map_size", string( ) ) );
        MapSize = UTIL_ExtractNumbers( CFG->GetString( "map_size", string( ) ), 4 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_info with config value map_info = " + CFG->GetString( "map_info", string( ) ) );
        MapInfo = UTIL_ExtractNumbers( CFG->GetString( "map_info", string( ) ), 4 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_crc with config value map_crc = " + CFG->GetString( "map_crc", string( ) ) );
        MapCRC = UTIL_ExtractNumbers( CFG->GetString( "map_crc", string( ) ), 4 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_sha1 with config value map_sha1 = " + CFG->GetString( "map_sha1", string( ) ) );
        MapSHA1 = UTIL_ExtractNumbers( CFG->GetString( "map_sha1", string( ) ), 20 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_filter_type with config value map_filter_type = " + CFG->GetString( "map_filter_type", string( ) ) );
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_options with config value map_options = " + CFG->GetString( "map_options", string( ) ) );
        MapOptions = CFG->GetInt( "map_options", 0 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_width with config value map_width = " + CFG->GetString( "map_width", string( ) ) );
        MapWidth = UTIL_ExtractNumbers( CFG->GetString( "map_width", string( ) ), 2 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_height with config value map_height = " + CFG->GetString( "map_height", string( ) ) );
        MapHeight = UTIL_ExtractNumbers( CFG->GetString( "map_height", string( ) ), 2 );
    }
    ...
    if( m_AliasType == "failed" ) {
        Log->Info( "Unrecognized AliasType, make sure you have set it correctly.");
    } else {
        Log->Info( "Successfully encoded AliasType, set Alias to ["+m_AliasType+"].");
    }
    if( MapNumPlayers == 0 )
        MapNumPlayers = CFG->GetInt( "map_numplayers", 0 );
    else if( CFG->Exists( "map_numplayers" ) )
    {
        //Log->Debug( "[MAP] overriding calculated map_numplayers with config value map_numplayers = " + CFG->GetString( "map_numplayers", string( ) ) );
        MapNumPlayers = CFG->GetInt( "map_numplayers", 0 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding calculated map_numteams with config value map_numteams = " + CFG->GetString( "map_numteams", string( ) ) );
        MapNumTeams = CFG->GetInt( "map_numteams", 0 );
    }
    ...
    {
        //Log->Debug( "[MAP] overriding slots" );
        Slots.clear( );
    ...
    if( m_MapFlags & MAPFLAG_RANDOMRACES )
    {
        //Log->Debug( "[MAP] forcing races to random" );
    ...
    {
        //Log->Debug( "[MAP] adding " + UTIL_ToString( 12 - m_Slots.size( ) ) + " observer slots" );
...
void CMap :: CheckValid( )
{
    ...
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_path detected" );
    }
    //else if( m_MapPath[0] == '\\' )
    //Log->Debug( "[MAP] warning - map_path starts with '\\', any replays saved by GHost++ will not be playable in Warcraft III" );

    //if( m_MapPath.find( '/' ) != string :: npos )
    //Log->Debug( "[MAP] warning - map_path contains forward slashes '/' but it must use Windows style back slashes '\\'" );

    if( m_MapSize.size( ) != 4 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_size detected" );
    }
    else if( !m_MapData.empty( ) && m_MapData.size( ) != UTIL_ByteArrayToUInt32( m_MapSize, false ) )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_size detected - size mismatch with actual map data" );
    }

    if( m_MapInfo.size( ) != 4 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_info detected" );
    }

    if( m_MapCRC.size( ) != 4 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_crc detected" );
    }

    if( m_MapSHA1.size( ) != 20 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_sha1 detected" );
    }

    if( m_MapSpeed != MAPSPEED_SLOW && m_MapSpeed != MAPSPEED_NORMAL && m_MapSpeed != MAPSPEED_FAST )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_speed detected" );
    }
    ...
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_visibility detected" );
    }
    ...
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_observers detected" );
    }
    ...
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_width detected" );
    }

    if( m_MapHeight.size( ) != 2 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_height detected" );
    }

    if( m_MapNumPlayers == 0 || m_MapNumPlayers > 12 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_numplayers detected" );
    }

    if( m_MapNumTeams == 0 || m_MapNumTeams > 12 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_numteams detected" );
    }

    if( m_Slots.empty( ) || m_Slots.size( ) > 12 )
    {
        m_Valid = false;
        //Log->Debug( "[MAP] invalid map_slot<x> detected" );
    }
}
